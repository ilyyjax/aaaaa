...
function spawnCloud(y) {
  ...
  cloud.style.left = Math.random() * (gameArea.clientWidth - 100) + 'px';
...
function spawnRainbow(y) {
  ...
  rainbow.style.left = Math.random() * (gameArea.clientWidth - 120) + 'px';
...
function activateTrail() {
  ...
  const trailInterval = setInterval(() => {
    ...
    spawnParticles(gameArea.clientWidth/2, gameArea.clientHeight/2);
  }, 100);
...
function gameLoop() {
  ...
  if (holding) {
    ...
    pickle.style.transform = `translateX(-50%) rotate(${angle}deg) scaleY(1.2) scaleX(0.8)`;
    pickleY += 3;
    score++;
    ...
    pickle.style.transform = `translateX(-50%) rotate(0deg) scaleY(1) scaleX(1)`;
  }
